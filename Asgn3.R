...
par(mfrow=c(1,1))
plot(alphas,elastic_model[,1],pch=16,main="cvmin", ylab = "cvmin")
plot(alphas,elastic_model[,2],pch=16,main="cvlo", ylab = "cvmin - standard error of cvm")
plot(alphas,elastic_model[,3],pch=16,main="cvup", ylab = "cvmin + standard error of cvm")


### The best model? It appears to be alpha = 0, which makes sense given the covariates are all distinct blood analytes from plasma samples of covid patients. 
optimal_alpha <- alphas[which.min(elastic_model[,1])]

#Now we need to determine the optimal lambda value (lambda.min) 
set.seed(1717)
...
correlation <- cor.test(x = covid_data$Age, covid_data$resp)
print(correlation)
...
severity_binary <- ifelse(covid_data$Severity == "Mild", 0, 1)

correlation <- cor.test(covid_data$AGE, as.numeric(covid_data$resp), method = "pearson")
